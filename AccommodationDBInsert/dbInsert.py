...
# 엑셀 파일 경로
excel_file_path = "xls/accommodation.xls"
...
data = data[['accommodation_type', 'country', 'bath', 'bed', 'bedroom', 'building_type', 'cost_per_night',
             'created_at', 'host_id', 'initial_discount_applied', 'max_guests', 'modified_at',
    ...
    with connection.cursor() as cursor:
        ...
        sql_accommodation = """
            INSERT INTO accommodation
                (accommodation_type, country, bath, bed, bedroom, building_type, cost_per_night,
                created_at, host_id, initial_discount_applied, max_guests, modified_at,
                monthly_discount_applied, weekly_discount_applied, coordinate,
                address, detailed_address, zipcode, description, name)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, ST_GeomFromText(%s, 4326), %s, %s, %s, %s, %s)
        """

        sql_discount = """
            INSERT INTO accommodation_discount
                (accommodation_id, remain_discount_cnt)
            VALUES (%s, %s)
        """
        ...
        for _, row in data.iterrows():
            cursor.execute(sql_accommodation, (
                ...
                row['initial_discount_applied'], row['max_guests'], row['modified_at'],
            ...
            # 첫할인 적용 시 accommodation_discount 테이블에 할인 횟수 추가
            if row['initial_discount_applied']:
                cursor.execute(sql_discount, (accommodation_id, 3))
